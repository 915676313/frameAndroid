...
public class ConvenientBanner<T> extends LinearLayout {
    ...
    private boolean canShowRoundImage = false;
    private float roundImageHeight;
    ...
    public ConvenientBanner(Context context, AttributeSet attrs) {
        ...
        canTurn = a.getBoolean(R.styleable.ConvenientBanner_canTurn, false);
        autoTurningTime = a.getInteger(R.styleable.ConvenientBanner_autoTurningTime, 0);
        canShowRoundImage = a.getBoolean(R.styleable.ConvenientBanner_canShowRoundImage, false);
        roundImageHeight = a.getDimension(R.styleable.ConvenientBanner_roundImageHeight, 32);
        a.recycle();
        init(context);
    }
    ...
    public ConvenientBanner(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        autoTurningTime = a.getInteger(R.styleable.ConvenientBanner_autoTurningTime, 0);
        canShowRoundImage = a.getBoolean(R.styleable.ConvenientBanner_canShowRoundImage, false);
        roundImageHeight = a.getDimension(R.styleable.ConvenientBanner_roundImageHeight, 0);
        a.recycle();
        init(context);
    }
    ...
    private void init(Context context) {
        View hView = LayoutInflater.from(context).inflate(
                R.layout.view_viewpager_banner, this, true);
        viewPager = (CBLoopViewPager) hView.findViewById(R.id.cbLoopViewPager);
        roundImage = (ImageView) hView.findViewById(R.id.roundImg);
        if (canShowRoundImage) {
            Logger.d("test", "roundImageHeight=" + roundImageHeight);
            roundImage.setVisibility(VISIBLE);
            ViewGroup.LayoutParams llp = roundImage.getLayoutParams();
            llp.height = (int) roundImageHeight;
            roundImage.setLayoutParams(llp);
            roundImage.setImageDrawable(new RoundDrawable(llp.height));
        } else {
            roundImage.setVisibility(GONE);
        }
        ...
        if (canTurn) {
            startTurning(autoTurningTime * 1000);
        }
    }
    ...
    public ConvenientBanner setPageIndicator(int[] page_indicatorId, boolean isRecyele) {
        this.isRecyele = isRecyele;
        setPageIndicator(page_indicatorId);
        if (isRecyele) {
            RelativeLayout.LayoutParams p = (RelativeLayout.LayoutParams) loPageTurningPoint.getLayoutParams();
            p.bottomMargin = DensityUtils.dp2px(getContext(), 10);
            for (int i = 1; i < loPageTurningPoint.getChildCount(); i++) {
                ((LayoutParams) loPageTurningPoint.getChildAt(i).getLayoutParams()).leftMargin
                        = DensityUtils.dp2px(getContext(), 4);
            }
        }
        return this;
    }
